...
# construct boundary
bdy1 = GSB(c=star(3*nb, a=0.0, r=2.5, f=11))
bdy2 = GSB(c=squish(nb, x=-1.2, y=-0.7, b=0.05, rot=-np.pi/4))
bdy3 = GSB(c=star(2*nb, x=0.8, y=0.5, a=0.3, f=3))
if reparametrize:
...
ebdyc.ready_bump(MOL.bump, (2.6-ebdys[0].radial_width, 2.6-ebdys[0].radial_width), ebdys[0].radial_width)
...
U_gerrp = np.sqrt(u_gerrp**2 + v_gerrp**2)
U_err = np.sqrt(np.array(u_r_errs)**2 + np.array(v_r_errs)**2)
be = max(U_gerrp.max(), U_err.max())
print('Biggest error: {:0.4e}'.format(be))


ns = 100*np.arange(1, 11)
uscale = np.sqrt(uc**2 + vc**2).max()
errors = [2.5864e-01, 1.9073e-03, 3.6029e-05, 4.8345e-07, 2.1437e-08, 2.0758e-09, 3.3441e-10, 6.1537e-10, 8.6307e-10, 7.5079e-10] / uscale
...
if False:
	...
	fig, ax = plt.subplots()
	ax.plot(ns, errors, color='black', linewidth=2, marker='^')
	ax.set_yscale('log')
	ax.set_xlabel(r'$n_\text{boundary}$')
	ax.set_ylabel(r'$\|\mathbf{u}_\text{analytic}-\mathbf{u}\|_{L^\infty(\Omega)}$')
	ax.axhline(1e-10, color='gray', linestyle='--')
	fig.tight_layout()
	fig.savefig('stokes_refinement.pdf', format='pdf', bbox_inches='tight')
