...
while t < max_time-1e-10:
	...
	for _ in range(eulerian_multiple):
		...
		if te == 0:
			...
			if timestepper == 'ab':
				ce_new = ce - 0.5*dt_eulerian*(3*ue*cxe + 3*ve*cye - uoe*cxoe - voe*cyoe + 3*scaled_nonlinearity(ce) - scaled_nonlinearity(coe))
				...
				raise Exception('Timestepper not found')
	...
	if t == 0:
		...
		# now interpolate to c
		if timestepper == 'ab':
			ch1 = ebdy.interpolate_to_points(c - 0.5*dt*3*scaled_nonlinearity(c), xd_all, yd_all, fix_r=True)
			ch2 = ebdy_old.interpolate_to_points(0.5*dt*scaled_nonlinearity(c_old), xD_all, yD_all, fix_r=True)
			ch = ch1 + ch2
		elif timestepper == 'bdf':
			...
			raise Exception('Timestepper not recognized.')
		# set the grid values
		c_new.grid_value[new_ebdyc.phys_not_in_annulus] = ch[:gp.N]
		# set the radial values
		c_new.radial_value_list[0][:] = ch[gp.N:].reshape(ebdy.radial_shape)
		...
		ffh = np.fft.rfft(ff)
		ffh[:,66:] = 0.0
		c_new.radial_value_list[0][:] = np.fft.irfft(ffh).real
...
err_r = c.radial_value_list[0] - cr
cerr = EmbeddedFunction(ebdyc)
cerr.load_data(err, [err_r,])
