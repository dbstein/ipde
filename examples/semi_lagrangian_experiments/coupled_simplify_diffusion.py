...
while t < max_time-1e-10:
	...
	if t == 0:
		...
		# construct embedded boundary
		new_ebdy = EmbeddedBoundary(new_bdy, True, M, bh*0.5, pad_zone, MOL.step, tolerances=tolerances)
		...
		if nu > 0:
			...
			A = s2_singular(new_ebdyc.ebdys[0].bdy) - half_eye(new_ebdyc.ebdys[0].bdy)
			bvn = solver2.get_boundary_normal_derivatives(c_new_update.radial_value_list)
			tau = -np.linalg.solve(A, -bvn)
			qfs = QFS_Evaluator(new_ebdyc.ebdys[0].bdy_qfs, True, [Singular_SLP2,], Naive_SLP2, on_surface=True, form_b2c=False)
			sigma = qfs([tau,])
...
if False:
	diffs_300_12 = [1.19e-02, 1.30e-03, 4.14e-04, 1.50e-02]

	diffs_200_8_only_diffusion = [4.22e-03, ]